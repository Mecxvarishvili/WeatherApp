...
export async function getWeather (lat: number, lon: number) {
    const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&timezone=auto&current=weather_code,temperature_2m,relative_humidity_2m,apparent_temperature,rain,showers,snowfall,cloud_cover,wind_speed_10m,uv_index&daily=weather_code,precipitation_hours,precipitation_probability_mean&hourly=temperature_2m,weather_code`)
    return res.json()
}
